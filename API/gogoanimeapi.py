...
class gogoanime():
    ...
    def get_search_results(query):
        try:
            url1 = f"https://gogoanime.com.so//search.html?keyword={query}"
            session = HTMLSession()
    ...
    def get_anime_details(animeid):
        try:
            animelink = 'https://gogoanime.com.so/category/{}'.format(animeid)
    ...
    def get_episodes_link(animeid, episode_num):
        try:
            animelink = f'https://gogoanime.com.so/category/{animeid}'
            response = requests.get(animelink)
            plainText = response.text
            soup = BeautifulSoup(plainText, "lxml")
            lnk = soup.find(id="episode_page")
            ...
            tit_url = soup.find("div", {"class": "anime_info_body_bg"}).h1.string
            URL_PATTERN = 'https://gogoanime.com.so/{}-episode-{}'
    ...
    def jugad(animeid, episode_num):
        url = f"https://www1.gogoanime.so/{animeid}-episode-{episode_num}"
